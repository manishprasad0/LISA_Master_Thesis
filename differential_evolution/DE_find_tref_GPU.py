...
from tools.MBHB_differential_evolution_GPU import MBHB_finder_time_frequency, transform_bbhx_to_parameters, transform_parameters_to_bbhx
...
def main():
    ...
    def pre_merger(gravitational_wave_data_t, time_before_merger, t_ref, t_array):
        cutoff_time = t_ref - time_before_merger
        cutoff_index = cp.searchsorted(t_array, cp.asarray(cutoff_time))
    ...
    differential_evolution_kwargs = {
        ...
        'maxiter': 200,            # give it more room for global exploration
    ...
    found_snr_found_tf = [found_snr_found_tf[0].get()]
    save_de_results(
        found_parameters_tf.get(),
        found_snr_found_tf,
        true_snr.get(),
        results_tf,
